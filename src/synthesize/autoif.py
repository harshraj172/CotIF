...
class AutoIf:
    ...
    def compile(
        ...
        if max_samples > 0 and max_samples < data_size:
            ...
            processing_dataset = dataset.select(range(1000, args.max_samples))
        else:
        ...
        for batch_start in range(0, len(processing_dataset), batch_size):
            ...
            # partial saving
            with open(args.output_file.replace(".jsonl", "") + "-preference.jsonl", "w") as f:
                for result in self.preference_data:
...
if __name__ == "__main__":
    ...
    # Run AutoIf
    preference_data, filtered_data = autoif.compile(
        subset, 
