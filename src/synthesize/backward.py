...
def using_template(dataset):
    ...
    for datum in tqdm(dataset, desc="Processing dataset (using templates)"):
        ...
        save_jsonl(modified_instructions[:int(0.9 * len(modified_instructions))], f"data/backward/data_1k-using_templates-train-1.jsonl", append=True)
        save_jsonl(modified_instructions[int(0.9 * len(modified_instructions)):], f"data/backward/data_1k-using_templates-test-1.jsonl", append=True)
